...
import { markMessagesAsRead } from '@/lib/server/issue-service';
...
export async function POST(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    if (!isAdmin && !isOwner) {
      return NextResponse.json({ error: 'Not authorized to access this issue' }, { status: 403 });
    }

    // Mark messages from the other party as read
    const senderToMark = isAdmin ? 'CUSTOMER' : 'ADMIN';
    const markedCount = await markMessagesAsRead(issueId, senderToMark);
